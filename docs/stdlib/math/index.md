...
| ---------- | --------------------------------------------------------------------------------------------- |
| **Module** | `math`                                                                                        |
| **Source** | [`math/math.zirr`](https://code.knabel.dev/zirric-lang/zirric/src/branch/main/math/math.zirr) |

`math` operates on [`Number`](../prelude/index.md#number), the union of [`Int`](../prelude/index.md#int) and [`Float`](../prelude/index.md#float). Functions that can only answer in fractions ([`sqrt`](#sqrt), [`floor`](#floor), [`pow`](#pow)) return `Float`; the ones that merely pick or transform a value ([`abs`](#abs), [`min`](#min), [`max`](#max), [`clamp`](#clamp)) return whichever kind they were given.
...
- **Constants** — [`pi`](#pi), [`e`](#e)
- **Functions** — [`toFloat`](#tofloat), [`toInt`](#toint), [`abs`](#abs), [`min`](#min), [`max`](#max), [`isNaN`](#isnan), [`isInfinite`](#isinfinite), [`sign`](#sign), [`floor`](#floor), [`ceil`](#ceil), [`round`](#round), [`trunc`](#trunc), [`sqrt`](#sqrt), [`pow`](#pow), [`clamp`](#clamp)
...
Returns whether v is not a number, as produced by an undefined operation such as 0.0 / 0.0 or the square root of a negative. An Int is never NaN. NaN is equal to nothing, itself included, so this is the only way to detect it.
...
- [`random`](../random/index.md) — drawing numbers instead of computing them.
- [Expressions § Operators](/specification/expressions) — the arithmetic built into the language.
