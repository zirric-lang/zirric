...
| ---------- | ----------------------------------------------------------------------------------------- |
| **Module** | `fun`                                                                                     |
| **Source** | [`fun/fun.zirr`](https://code.knabel.dev/zirric-lang/zirric/src/branch/main/fun/fun.zirr) |
...
The second is [`map`](#map), [`filter`](#filter), [`flatMap`](#flatmap), [`take`](#take), [`skip`](#skip) and [`zip`](#zip) over anything carrying [`Iterable`](../prelude/index.md#iterable). These are lazy: they build a new iterable and nothing runs until something consumes it — a `for` loop, [`reduce`](#reduce), or any other consumer. That is what lets [`take`](#take) stop an otherwise unbounded sequence.
...
- **Data** — [`Zipped`](#zipped)
- **Functions** — [`with`](#with), [`pipe`](#pipe), [`identity`](#identity), [`constant`](#constant), [`negate`](#negate), [`compose`](#compose), [`flip`](#flip), [`map`](#map), [`filter`](#filter), [`flatMap`](#flatmap), [`reduce`](#reduce), [`take`](#take), [`skip`](#skip), [`zip`](#zip)
...
Returns a lazy @Iterable of Zipped(x, y) for each x in a paired with the element at the same position in b, stopping as soon as either source is exhausted. b is buffered into memory up front — pairing needs random access into it, which the push-based @Iterable protocol can't give incrementally for two independent sources — so b must be finite, even though a is still walked one element at a time.

---

## See also

- [`arrays`](../arrays/index.md), [`dicts`](../dicts/index.md) — eager versions over concrete collections.
- [`prelude`](../prelude/index.md#iterable) — the `Iterable` attribute everything here consumes.
